import { eq, or, and, ne, like, desc, sql, gte, lte } from "drizzle-orm";
  ...
  if (userId && userId !== "all") {
    conditions.push(
      or(
        ...
        sql`JSON_CONTAINS(${directFeasibilities.collaborators}, CAST(${JSON.stringify([userId])} AS JSON), '$')`
      )
    );
  }
  if (query) {
    conditions.push(like(directFeasibilities.name, `%${query}%`));
  }
...
export async function getAllDirect(req, res) {
  try {
    ...
    const countQuery = filter
      ? db.select({ count: sql`count(*)` }).from(directFeasibilities).where(filter)
      : db.select({ count: sql`count(*)` }).from(directFeasibilities);
...
export async function getAnalytics(req, res) {
  try {
    ...
    if (searchQuery) {
      const searchCond = like(directFeasibilities.name, `%${searchQuery}%`);
...
export async function getMyFeasibilities(req, res) {
  try {
    ...
    if (searchQuery) conditions.push(like(directFeasibilities.name, `%${searchQuery}%`));
    const filter = and(...conditions);

    const [countResult] = await db.select({ count: sql`count(*)` }).from(directFeasibilities).where(filter);
...
export async function getCollaborativeFeasibilities(req, res) {
  try {
    ...
    const [countResult] = await db.select({ count: sql`count(*)` }).from(directFeasibilities).where(fullFilter);
    const totalTemplates = Number(countResult?.count ?? 0);
