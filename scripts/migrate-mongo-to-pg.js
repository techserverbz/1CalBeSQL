 ...
 * transform to Drizzle schema shape, and insert into the app database in FK order.
 * Supports MySQL (and previously Postgres); DATABASE_URL must point to the target DB.
 *
 * Usage:
 ...
 *   DATABASE_URL must be set (e.g. MySQL or Postgres connection string).
...
async function migrateUsers(dir, idMaps) {
  ...
  for (let i = 0; i < out.length; i += BATCH_SIZE) {
    ...
    await db.insert(users).values(chunk).onDuplicateKeyUpdate({ set: { id: sql`id` } });
  }
...
async function migrateTemplates(dir, idMaps) {
  ...
  for (let i = 0; i < out.length; i += BATCH_SIZE) {
    await db.insert(templates).values(out.slice(i, i + BATCH_SIZE)).onDuplicateKeyUpdate({ set: { id: sql`id` } });
  }
...
async function migrateVersions(dir, idMaps) {
  ...
  idMaps.versions = map;
  for (let i = 0; i < out.length; i += BATCH_SIZE) {
    await db.insert(versions).values(out.slice(i, i + BATCH_SIZE)).onDuplicateKeyUpdate({ set: { id: sql`id` } });
  }
...
async function migrateDirectFeasibilities(dir, idMaps) {
  ...
  for (let i = 0; i < out.length; i += BATCH_SIZE) {
    await db.insert(directFeasibilities).values(out.slice(i, i + BATCH_SIZE)).onDuplicateKeyUpdate({ set: { id: sql`id` } });
  }
...
async function migrateContacts(dir, idMaps) {
  ...
  for (let i = 0; i < out.length; i += BATCH_SIZE) {
    await db.insert(contacts).values(out.slice(i, i + BATCH_SIZE)).onDuplicateKeyUpdate({ set: { id: sql`id` } });
  }
...
async function migrateAboutUs(dir) {
  ...
  for (let i = 0; i < out.length; i += BATCH_SIZE) {
    await db.insert(aboutUs).values(out.slice(i, i + BATCH_SIZE)).onDuplicateKeyUpdate({ set: { id: sql`id` } });
  }
...
async function migrateBills(dir, idMaps) {
  ...
  for (let i = 0; i < out.length; i += BATCH_SIZE) {
    await db.insert(bills).values(out.slice(i, i + BATCH_SIZE)).onDuplicateKeyUpdate({ set: { id: sql`id` } });
  }
...
async function migratePdfDownloadLogs(dir, idMaps) {
  ...
  for (let i = 0; i < out.length; i += BATCH_SIZE) {
    await db.insert(pdfDownloadLogs).values(out.slice(i, i + BATCH_SIZE)).onDuplicateKeyUpdate({ set: { id: sql`id` } });
  }
...
async function migrateFileTemplates(dir) {
  ...
  for (let i = 0; i < out.length; i += BATCH_SIZE) {
    await db.insert(fileTemplates).values(out.slice(i, i + BATCH_SIZE)).onDuplicateKeyUpdate({ set: { id: sql`id` } });
  }
